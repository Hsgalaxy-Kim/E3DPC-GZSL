...
class Trainer_default():
    ...
    def set_generator(self, embed_feature_size, num_classes=21):
        if self.args.generator_model == "gmmn":
            self.generator = GMMNnetwork(self.args.noise_dim, self.args.embed_dim,\
                self.args.hidden_size, self.args.feature_dim, embed_feature_size=embed_feature_size, num_classes=num_classes)
    ...
    def validation_metric_logs(self, test_loss, epoch, num):
        ...
        print("hIoU:{:.3f}".format((2*mIoU_seen*mIoU_unseen)/(mIoU_seen+mIoU_unseen)))
