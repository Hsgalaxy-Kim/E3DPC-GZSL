...
from utils.loss import SegmentationLosses, InfoNCELosses, Evidence_loss
    ...
    def training(self, epoch, args):
        ...
        for i, sample in enumerate(tbar):
            if len(sample[0]) > 1:
                ...
                ori_loss = self.criterion(output.view(-1, self.N_CLASSES), target.view(-1))
                
                edl_output = self.head(fake_features.detach())
                edl_output = edl_output.view(-1, self.N_CLASSES)
                alpha = torch.exp(torch.clamp(edl_output, -10, 10)) + 1
                edl_loss_ce = self.edl_ce(alpha[bi_labels==0], target.view(-1)[bi_labels==0])
                S = torch.sum(alpha[:, args.seen_classes_idx_metric], dim=-1)
                uncertainty = (len(args.seen_classes_idx_metric) / S)
                edl_loss_bce = F.binary_cross_entropy(uncertainty, bi_labels) * 0.1
    ...
    def validation(self, epoch, args):
        ...
        for i, sample in enumerate(tbar):
            ...
            with torch.no_grad():
                ...
                backbone_feat = self.model.module.backbone(features, pts, return_features=True)
                backbone_feat = backbone_feat.view(-1, 128)
                output = self.model.module.fcout_gen(backbone_feat).view(-1, 8192, self.N_CLASSES)
