...
from seg.utils.loss import SegmentationLosses, InfoNCELosses, Evidence_loss
...
class Trainer(Trainer_default):
    ...
    def __init__(self, args):
        ...
        self.config.path = args.data_path
        ...
        self.set_generator(embed_feature_size=embed_feature_size, num_classes=self.N_CLASSES)

        self.binary_labels = torch.zeros((self.N_CLASSES)).cuda()
        for l in args.unseen_classes_idx_metric:
            if l-1 >=0:
                self.binary_labels[l-1] = 1
        ...
        shifted_unseen_classes_idx_metric = shifted_unseen_classes_idx_metric[1:]
        self.shifted_seen_classes_idx_metric = shifted_seen_classes_idx_metric
        self.shifted_unseen_classes_idx_metric = shifted_unseen_classes_idx_metric
        ...
        EDL_loss = Evidence_loss(num_classes=self.N_CLASSES, seen_idx=shifted_seen_classes_idx_metric)
    ...
    def training(self, epoch, args):
        ...
        for i, sample in enumerate(tbar):
            ...
            for count_sample_i, lenght_i in enumerate(sample.lengths[0]):
                ...
                shifted_unique = self.model.module.labels_adaption(unique_class)
                
                sample_group = torch.full((real_features_i.shape[0], self.N_CLASSES), -1, dtype=torch.float32).cuda()
                sample_group[:, shifted_unique] = 1
            ...
            bi_labels = self.binary_labels[target_adapted.view(-1)]
            fake_features = fake_features.permute(1, 0).contiguous()
            output = self.model.module.training_generative(x=fake_features.detach())
            
            ori_loss = self.criterion(output.view(-1, self.N_CLASSES), target_adapted.view(-1))
            
            edl_output = self.head(fake_features.detach())
            alpha = torch.exp(torch.clamp(edl_output, -10, 10)) + 1
            edl_loss_ce = self.edl_ce(alpha[bi_labels==0], target_adapted.view(-1)[bi_labels==0])
            S = torch.sum(alpha[:, args.seen_classes_idx_metric], dim=-1)
            uncertainty = (len(args.seen_classes_idx_metric) / S)
            edl_loss_bce = F.binary_cross_entropy(uncertainty, bi_labels) * 0.005
            
            kl_weight = 0.05
            edl_loss_kl_seen = self.edl_kl(alpha[bi_labels==0], target_adapted.view(-1)[bi_labels==0]) * kl_weight
            EDL_KLS_loss += edl_loss_kl_seen.item()
            unique_class = torch.unique(target_adapted.view(-1))
            has_unseen_class = False
            for u_class in unique_class:
                if u_class in self.shifted_unseen_classes_idx_metric:
                    has_unseen_class = True
            if has_unseen_class:
                edl_loss_kl_unseen = self.edl_kl_unseen(alpha[bi_labels==1], target_adapted.view(-1)[bi_labels==1]) * kl_weight
    ...
    def validation(self, epoch, args):
        ...
        for i, sample in enumerate(tbar):
            ...
            with torch.no_grad():
                ...
                backbone_feat, _ = self.model.module.backbone(batch=sample, config=self.config)
                output = self.model.module.training_generative(backbone_feat)
                loss = self.criterion(output.view(-1, self.N_CLASSES), target_adapted.view(-1))
                
                edl_output = self.head(backbone_feat)
                alpha = torch.exp(torch.clamp(edl_output, -10, 10)) + 1
                alpha = alpha[:, self.shifted_seen_classes_idx_metric]
