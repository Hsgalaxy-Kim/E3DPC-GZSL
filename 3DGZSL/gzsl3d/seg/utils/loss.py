...
class Evidence_loss:
    def __init__(self, num_classes, seen_idx):
        self.num_classes = num_classes
        self.seen_idx = seen_idx
        self.seen_classes = len(seen_idx)

    def EDL_CE(self, alpha, target):
        y = F.one_hot(target, self.num_classes)
        y = y[:, self.seen_idx]
        
        alpha = alpha[:, self.seen_idx]
        
        S = torch.sum(alpha, dim=-1)
        diga_S = torch.unsqueeze(torch.digamma(S), dim=1).repeat(1, self.seen_classes)
        diga_alpha = torch.digamma(alpha)
        loss = torch.sum(y * (diga_S - diga_alpha)) / y.shape[0]
        
        return loss
        
    def EDL_KL_v2(self,alpha, target):
        y = F.one_hot(target, self.num_classes)
        y = y[:, self.seen_idx]
        
        alpha = alpha[:, self.seen_idx]
        modified_alpha=y+(1-y)*torch.sqrt(alpha) + 1 / torch.sqrt(alpha)*y
        
        device = modified_alpha.device
        ones = torch.ones([1, self.seen_classes], dtype=torch.float32, device=device)
        ...
        kl = torch.sum(kl) / y.shape[0]
        return kl
    
    def EDL_KL_unseen(self,alpha, target):
        y = F.one_hot(target, self.num_classes)
        y = y[:, self.seen_idx]
