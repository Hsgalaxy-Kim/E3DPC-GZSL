...
#import valeodata
def nearest_correspondance(pts_src, pts_dest, data_src, uncertainty, num, K=1):
    indices = nearest_neighbors.knn(pts_src.astype(np.float32), pts_dest.astype(np.float32), K, omp=True)
    if K == 1:
        indices = indices.ravel()
        data_dest = data_src[indices]
        knn_uncertainty = uncertainty[indices]
        knn_num = num[indices]
    else:
        data_dest = data_src[indices].mean(1)
        knn_uncertainty = uncertainty[indices].mean(1)
        knn_num = num[indices].mean(1)
    return data_dest, knn_uncertainty, knn_num
...
class PartDatasetTest():
    ...
    def __init__(self, pts, labels,
        ...
        self.pts = self.pts.astype(float) * step + mini + step / 2
...
def test(args, config):
    ...
    head = Final_Head()
    chkp_u_path = os.path.join(args.ckpt_u_path)
    print("Load the complete trained uncertainty estimator from {}".format(chkp_u_path))
    checkpoint_u = torch.load(chkp_u_path)
    head.load_state_dict(checkpoint_u["head"])
    ...
    seen_idx = [1, 2, 3, 4, 6, 9, 10, 12, 13, 14, 15, 16, 17, 18, 19, 20]
    unseen_idx = [0, 5, 7, 8, 11]
    ...
    for i in range(object_list_len):
        ...
        u = np.zeros((xyzrgb.shape[0]))
        num = np.zeros((xyzrgb.shape[0]))
        ...
        with torch.no_grad():
            ...
            for data in t:
                ...
                backbone_feats = model.backbone(x=x, pos=pos, support_points=net_pts, indices=net_ids, gen_forward=True)
                edl_output = head(backbone_feats)
                edl_output = edl_output.permute(0, 2, 1).contiguous().view(-1, N_CLASSES)
                
                alpha = torch.exp(torch.clamp(edl_output, -10, 10)) + 1
                alpha = alpha[:, seen_idx]
                S = torch.sum(alpha, dim=-1)
                uncertainty = (len(seen_idx) / S)
                uncertainty_np = uncertainty.cpu().numpy()

                t2 = time.time()
                scores[indices.cpu().numpy().ravel()] += outputs_np
                u[indices.cpu().numpy().ravel()] += uncertainty_np
                num[indices.cpu().numpy().ravel()] += 1
        ...
        scores, u, num = nearest_correspondance(pts_src, xyzrgb, scores, u, num, K=1)
        u = u / num
        ...
        unseen_mean_list = []
        for i in unseen_idx:
            if u[scores.argmax(1)==i].shape[0] == 0:
                continue
            mean = u[scores.argmax(1)==i].mean()
            unseen_mean_list.append(mean)
        unseen_mean = np.array(unseen_mean_list).mean()
        u -= unseen_mean
        
        scores[:, seen_idx] -= np.expand_dims(u, axis=1)        
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt_u_path', type=str, help="Uncertainty estimator path")
    ...
    torch.manual_seed(pars_args.seed)
    random.seed(pars_args.seed)
    os.environ['PYTHONHASHSEED'] = str(pars_args.seed)
    np.random.seed(pars_args.seed)
    torch.cuda.manual_seed(pars_args.seed)
    torch.cuda.manual_seed_all(pars_args.seed)
