...
def get_model(model_name, input_channels, output_channels, return_ll, args):
    ...
    if model_name == "SegBig":
        ...
        from gzsl3d.convpoint.networks.network_seg_test import SegBig as Net
...
def test(args, flist_test):
    head = Final_Head(type='s3dis', num_classes=13)
    ...
    if args.use_zsl_head:
        ...
        checkpoint_u = torch.load(args.zsl_trained_u_path)
    ...
    for filename in flist_test:
        ...
        with torch.no_grad():
            ...
            for pts, features, indices in t:
                ...
                if args.use_zsl_head:
                    ...
                    backbone_feats = net.backbone(features, pts)
                    backbone_feats = backbone_feats.view(-1, 128)
        ...
        scores[:, unseen_idx] += np.expand_dims(u, axis=1)
...
def main():
    ...
    parser.add_argument("--zsl_trained_u_path", type=str)
